...
frappe.ui.form.on('Footer Component', {
    ...
    section_html: function(frm, field, html, device_type) {
        ...
        if (frm.doc[field] && frm.doc[field].length > 0) {
            frm.doc[field].map(f => {
                ...
                var col_sec = $(`<input type="number" class="form-control" data-index="${f.idx}" value="${f.column_index}"/>`);
                $(row).find('.col-idx').append(col_sec);
                col_sec.change(()=>{
                    var p_type = frm.doc.page_type;
                    var idx = $(col_sec).attr("data-index");
                    cur_frm.get_field("web_section").grid.data[idx-1].column_index=$(col_sec).val()
                ...
                if (f.section_type != 'Slider')
                    ...
                    let slider_link = $(`<button class="btn btn-primary btn-xs" style="margin-right: 10px;"><span class="fa fa-edit"></span></button>`);
                ...
                if (allow) {
                    let modify_data = $(`<button class="btn btn-primary btn-xs" style="margin-right: 10px;"><span class="fa fa-edit"></span></button>`);
                ...
                if (device_type == 'Mobile') {
                    let modify_design = $(`<button class="btn btn-info btn-xs" style="margin-right: 10px;"><span class="fa fa-code"></span></button>`);
                ...
                let delete_section = $(`<button class="btn btn-danger btn-xs"><span class="fa fa-trash"></span></button>`);
        ...
        let create_section = $(`<button class="btn btn-sm btn-primary" style="margin-left: 15px;">Add New Section</button>`).appendTo(wrapper);
        // add_section.click(()=>{
        //     new add_new_section({
        //         parentfield: field,
    ...
    pick_layout(frm){
		frappe.call({
			method:"computroidcms.computroidcms.doctype.footer_component.footer_component.get_footer_layouts",
			args:{},
			freeze:true,
			callback:(res) =>{
    ...
    open_footer_popoup(frm,layouts){
		...
		$(layout_dialogs.$wrapper).find(".modal-dialog").css({"min-width":"70%"})
		layout_dialogs.show()
		...
		let p_add_html = ""
		layouts.map(each_p =>{
			p_add_html += `<div class="each-palette-div" style="flex:0 0 calc(25% - 20px);position:relative;">
		<label style="cursor:pointer;width:100%;" value="${each_p.name}">
			<div style="width:170px;height:54px;margin:auto;
				background-image: url(${each_p.preview ? "'" + each_p.preview.replace(" ",'%20') + "'" : '/files/no-img.jpg'});
			...
			<input type="radio" value="${each_p.name}" name="theme_radio" onclick="select_unselect_layout($(this))" style="position:absolute;top:0px;left:20px;">
		</label></div>`
...
var add_new_section = Class.extend({
    ...
    get_section_templates: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_footer_section_templates',
            args: {
                device_type: me.device_type
            },
            async: false,
    ...
    make_dialog: function() {
        ...
        this.dialog.$wrapper.find('.form-section').attr("style","width: calc(100% + 6px);margin-left: -7px;padding-top: 0;margin-top: 0;");
    ...
    section_html: function() {
       ...
       if(me.groups.length>0){
        ...
        if(me.groups.length>0){
             wrapper.find('#SectionList').append("<div class='col-md-3' id='sec-left-content' style='background-color: #fff;min-height: 547px;margin-left: -15px;position: fixed;left: 1.5%;border-right: 1px solid #ddd;'></div>");
             wrapper.find('#SectionList').append("<div class='col-md-9' id='sec-right-content' style='padding-top:20px;margin-left:25%'></div>");
             ...
             group_html+="<ul style='float: left;width: 100%;padding: 0;margin: 0;'><li style='list-style:none;float: left;width: calc(100% + 30px);padding: 10px 15px;border-bottom: 1px solid #ddd;margin-left: -15px;'><a class='group-link' onclick=filter_section('All')>All</a></li>";
             for (var i = 0; i < me.groups.length; i++) {
                 group_html+='<li style="list-style:none;float: left;width: calc(100% + 30px);padding: 10px 15px;border-bottom: 1px solid #ddd;margin-left: -15px;"><a class="group-link" style="float: left;width: 100%;text-decoration: none;"';
            ...
            wrapper.find('#SectionList').append("<div class='col-md-12' id='sec-right-content' style='padding-top:20px'></div>");
        }
        data.map(f => {
        ...
        // }
         if(this.fields.allow_update_to_style==1){
            if(this.fields.styles.length>1){
                ...
                this.dialog.$wrapper.find('.form-layout').css("background", "#fff");
                this.dialog.$wrapper.find('.form-layout').css("width", "calc(100% + 30px)");
                this.dialog.$wrapper.find('.row.form-section.visible-section').css("width", "70%");
                 this.dialog.$wrapper.find('.row.form-section.visible-section').css("float", "left");
                this.dialog.$wrapper.find('.row.form-section.visible-section:first-child').css("width", "30%");
                this.dialog.$wrapper.find('.row.form-section.visible-section:first-child').css("border-right", "1px solid #ddd");
                this.dialog.$wrapper.find('.row.form-section.visible-section:first-child').css("min-height", "720px");
                this.dialog.$wrapper.find('.row.form-section.visible-section:first-child').find("h6").attr("style","margin-left: -20px;margin-top: -15px;background-color: #f5f5f5;width: calc(100% + 42px);padding: 10px 15px;")
    ...
    get_fields_list: function() {
        ...
        if(this.fields.allow_update_to_style==1){
            if(this.fields.styles.length>1){
                ...
                field_list.push({ 'fieldtype': 'Section Break', 'fieldname': 'sb_design', 'label': 'Design', 'collapsible': 0 })
        ...
        $.each(me.dialog.fields, function(k, v) {
            if(v.fieldtype != "Section Break"){
                if(jQuery.inArray(v.fieldname, dialog_fields) === -1) {
                    if (me.fields.section_type != 'Lists' && me.fields.section_type != 'Tabs')
                        results.push({ 'name': v.fieldname, 'content': '', 'doctype': 'Section Content', 'parent': me.section });
                    else if (me.fields.section_type == "Lists" && !has_common([k], ['item', 'item_title', 'is_edit', 'image_type', 'image_ref_doc']))
                        results.push({ 'name': v.fieldname, 'content': '', 'doctype': 'Section Content', 'parent': me.section });
                    else if (me.fields.section_type == 'Tabs' && !has_common([k], ['tab_item', 'tab_title', 'is_edit_tab', 'no_of_records', 'item1', 'item_title1', 'is_edit1'])) {
                        results.push({ 'name': v.fieldname, 'content': '', 'doctype': 'Section Content', 'parent': me.section });
