...
def get_path_name():
	...
	if "computroidcms" in frappe.get_installed_apps():
		path = frappe.get_app_path("computroidcms")
		# frappe.log_error(path,">> path <<")
		sitename = 'computroidcms'
...
def generate_webtheme_css_file(path,sitename,self):
	try:
		...
		for page in pages:
			...
			if page.edit_header_style == 1:
				if page.is_transparent_header == 1:
					...
					if "/" in p_route:
						p_route = p_route.split('/')[1]
					webtheme_css+="."+p_route+" .computroid-cms-header{background:transparent;}"
					webtheme_css+="."+p_route+" .computroid-cms-menu li a{color:"+page.menu_text_color+";}"
					if page.menu_hover_bg:
						webtheme_css+="."+p_route+" .computroid-cms-menu li a:hover{background-color:"+page.menu_hover_bg+";}"
					if page.menu_hover_text_color:
						webtheme_css+="."+p_route+" .computroid-cms-menu li a:hover{color:"+page.menu_hover_text_color+";}"
					if header_settings:
						webtheme_css+="."+p_route+" .sticky_header .computroid-cms-menu li a{color:"+header_settings.menu_text_color+";}"
						webtheme_css+="."+p_route+" .sticky_header .computroid-cms-menu li a:hover{color:"+header_settings.menu_hover_color+";}"
					else:
						if is_custom_header:
							if doc_obj.header_settings:
								webtheme_css+="."+p_route+" .sticky_header .computroid-cms-menu li a{color:"+doc_obj.header_settings.menu_text_color+";}"
								webtheme_css+="."+p_route+" .sticky_header .computroid-cms-menu li a:hover{color:"+doc_obj.header_settings.menu_hover_color+";}"
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.generate_webtheme_css_file")
...
def get_cms_json():
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.get_cms_json")
...
def get_json_render_properties(field_type):
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.get_json_render_properties")
...
def make_random_class_name():
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.make_random_class_name")
...
def get_color_palette_datas():
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.get_color_palette_datas")
...
def get_color_palette_default_datas(palette_id):
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.get_color_palette_default_datas")
# end
...
def get_color_code_data(bg_type):
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.web_theme.web_theme.def get_color_code_data")
# end
