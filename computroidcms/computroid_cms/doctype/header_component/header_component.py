...
class HeaderComponent(Document):
	def on_update(self):
...
def get_header_layouts():
	try:
		final_data=[]
		all__data = frappe.db.get_list("Header Layout",fields=["name","title","preview","layout_json"],filters={"is_active":1})
		for k in all__data:
			final_data.append({"layout_json":json.loads(k.layout_json),"preview":k.preview,"title":k.title,"name":k.name})
		return final_data
	except Exception:
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.doctype.footer_component.footer_component.get_footer_layouts")