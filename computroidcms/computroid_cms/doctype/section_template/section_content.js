frappe.ui.form.on('Section Content', 'form_render', function (frm, cdt, cdn) {
    ...
    $('.btn-default[data-fieldname="choose_style_properties"]').css("border","1px solid #ddd");
    $('.btn-default[data-fieldname="setup_defaults"]').css("border","1px solid #ddd");
...
frappe.ui.form.on('Section Content', {
    ...
    setup_defaults:function(frm,cdt,cdn){
      frappe.call({
                method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_section_element_properties',
                args: {"id":cdn},
                async: false,
                callback: function(r) {
                    if(r.message.css_properties_list){
                       ...
                       styledialog.set_primary_action(__('Save'), function() {
                           ...
                           for (let k in values) {
                            ...
                            styledialog.hide();
                            frappe.model.set_value(cdt,cdn,'css_json',JSON.stringify(style_json))
                            // $(".modal-dialog[id='"+cdn+"']").parent().remove();
                        });
                       for (var i = 0; i < child_sections.length; i++) {
                           // if(i!=child_sections.length-1){
                           styledialog.$wrapper.find(".frappe-control[data-fieldname='"+child_sections[i]+"']").parent().parent().parent().parent().attr("style","margin-bottom:10px;border-top:none !important;margin-top: -20px;");//modfied by boopathy
    ...
    choose_style_properties:function(frm, cdt, cdn){
        ...
        frappe.call({
            method:"computroidcms.computroidcms.doctype.section_template.section_template.get_css_fields",
            args:{},
            callback:function(r){
...
function fields_json(item, html) {
    ...
    $(`<div><button class="btn btn-sm btn-primary">${__("Add Fields")}</button><button class="btn btn-sm btn-warning btn-add-edit-data" style="margin-left:15px">${__("Add / View Data")}</button></div>
    ...
    if (fields_list && fields_list.length > 0) {
        fields_list.map(f => {
            ...
            row.find('.btn-danger').click(() => {
                ...
                frappe.model.set_value(item.doctype, item.name, 'fields_json', JSON.stringify(new_list));
                if (!cur_frm.__islocal)
                    cur_frm.save();
                else
                    fields_json(item, html);
            });
        })
    } else {
        $(wrapper).find('tbody').append(`<tr><td colspan="4">${__("No records found!")}</td></tr>`);
    }
                ...
                if(values.length > 0) {
                    values.map((f,index) => {
                        var row_html = '<tr>';
                         for (var i = 0; i < fields.length; i++) {
                            if(fields[i].field_type!="Attach"){
                               ...
                               row_html+='<td style="border-color: #ddd;"><img style="max-height:50px" src="'+f[fields[i].field_key]+'"/></td>';
                        ...
                        row.find('.btn-danger').click(function() {
                            ...
                            frappe.model.set_value('Section Content', doc.name, 'content',  JSON.stringify(updated_date))
                            view_data_dialog.$wrapper.find('#list_html_table tbody tr:eq('+index+')').remove();
                        })
...
function get_fields_list(wrapper, doc, frm_type, edit_doc) {
    ...
    dialog.set_primary_action(__("Save"), () => {
        ...
        if (doc.fields_json)
            prev_vals = JSON.parse(doc.fields_json);
        if (frm_type == 'Add') {
        ...
        frappe.model.set_value(doc.doctype, doc.name, 'fields_json', JSON.stringify(prev_vals));
        dialog.hide();
...
cms.SectionBuilder = Class.extend({
    ...
    get_layouts: function () {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.api.get_section_layouts',
            args: {},
            callback: function (r) {
                if (r.message) {
    ...
    get_components: function () {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.api.get_section_components',
            args: {},
            callback: function (r) {
                if (r.message) {
    ...
    get_css_properties: function () {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.api.get_styles_list',
            args: {},
            callback: function (r) {
                if (r.message) {
    ...
    make_section_layout: function () {
        let title = "";
        if (this.from_form == 1 && this.title_field)
        ...
        <style>
            ...
            div[data-fieldname="${this.field_name}"] .section-row-layout .new-row-div{
                text-align: center;
                padding: 10px;
                margin: 10px;
            }
            div[data-fieldname="${this.field_name}"] .section-row-layout .add-row{
                padding: 5px 10px;
                background: #4d8a4d;
                border-radius: 50%;
                color: #fff;
        ...
        this.make_settings(settings_wrapper, ['Settings'], 'Section', '#{{section}} .sectiontemplate');
        this.make_row();
    },
    make_settings: function (wrapper, settings_list, type, selector, row_count, column_count, component_class) {
        let me = this;
        if (settings_list && settings_list.length > 0) {
            $(settings_list).each(function (k, v) {
                ...
                row.click(()=>{
                    ...
                    if(v == 'Copy'){
                        me.copy_element(wrapper, type, selector, row_count, column_count, component_class);
                    }
                    if(v == 'Delete'){
                        me.delete_element(wrapper, type, selector, row_count, column_count, component_class)
                    }
                });
            });
        }
    ...
    settings_modal: function(type, selector){
        ...
        this.settings_dialog.set_primary_action(__('Update'), function(){
            ...
            $.each(values, function(k, v){
                let field_info = me.settings_dialog.fields.find(obj => obj.fieldname == k);
            ...
            me.style_template.appendChild(document.createTextNode(selector + ' {' + styles + '}'));
            me.save_template();
    ...
    settings_dialog_fields: function (type) {
        ...
        $(me.css_groups).each(function (k, v) {
            if (v.style_elements && v.style_elements.length > 0) {
                fields.push({ fieldname: 'cssgrp' + (k + 1), fieldtype: 'Section Break', label: __(v.name), collapsible: 1 });
                $(v.style_elements).each(function (key, val) {
                    let field = {};
                    field.fieldtype = val.value_type;
                    if (val.value_type == 'Number')
                        field.fieldtype = 'Float';
                    if (val.value_type == 'Attach')
                    ...
                    fields.push(field);
                    if ((key + 1) % 3 == 0) {
                        fields.push({ fieldname: 'colbr' + (k + 1), fieldtype: 'Column Break' });
                    }
                });
                    ...
                    $(style_list).each(function(i, j){
                        ...
                        try{
                            if(splits[1].indexOf('url') > -1) {
                                me.settings_dialog.set_value(key, splits[1].split('(')[1].split(')'));
                            } else {
                ...
                if (node.className.indexOf('row') != -1) {
                    node_html = $(`<div class="row-body-layout">
                        <div class="row-head">
                            <div class="row">
                                ...
                                <div class="col-md-6 row-title">${__("Row")}</div>
                                <div class="col-md-3"></div>
                            </div>
                        </div>
                    ...
                    if ($(node)[0].children.length > 0) {
                        for (var j = 0; j < $(node)[0].children.length; j++) {
                            let col_html = ''
                            if ($(node)[0].children[j].className.indexOf('section-col') != -1) {
                                        ...
                                        if (component.className.indexOf('section-comp') != -1) {
                                            let comp_html = $(`<div class="comp-head">
                                                <div class="row">
                                                    ...
                                                    <div class="col-md-6 comp-title">${__($(component).attr('data-component'))}</div>
                                                    <div class="col-md-3"></div>
                                                </div>
                                            ...
                                            me.make_settings(comp_settings, ['Settings', 'Copy', 'Delete'], 'Component', '#{{section}} ' + cls + ' ' + component_cls, (i + 1), (j + 1), component_cls);
                                        }
                                    }
                                }
                    ...
                    me.make_settings(row_settings, ['Settings', 'Copy', 'Delete'], 'Row', '#{{section}} .section-row', (i + 1));
                }
    ...
    layout_dialog: function (row_count) {
        ...
        let layout_wrapper = this.layout.fields_dict.layout_section.$wrapper.empty();
        ...
        let selected_layout;
        this.layouts.map(f => {
            let html = $(`<div class="col-md-6" style="float:left;"><div class="row"></div></div>`);
            let cols = f.web_layout.split('x');
            $(cols).each(function (k, v) {
                $(html).find('.row').append(`<div class="col-div col-md-${v.trim()}"><div></div></div>`);
            });
        ...
        this.layout.set_primary_action(__("Save"), function () {
            if (selected_layout) {
                let new_row = $(`<div><div class="row section-row sec-row${row_count}"></div></div>`);
                let cols = selected_layout.web_layout.split('x');
                $(cols).each(function (k, v) {
                    let div = `<div class="col-md-${v.trim()} section-col sec-row${row_count}-col${(k + 1)}"></div>`;
                ...
                $(me.wrapper).find('.section-row-layout').find('.new-row-div').parent().parent().remove();
                me.save_template();
                me.make_row();
                me.layout.hide();
            } else {
    ...
    add_new_component: function (class_name, row_count, column_count) {
        ...
        let component_wrapper = this.components_dialog.fields_dict.component_html.$wrapper.empty();
        ...
        this.components_dialog.set_primary_action(__("Add"), function () {
            if (selected_component) {                
                ...
                me.update_component(selected_component, row_count, column_count, new_cls_name);
                me.save_template();
                me.make_row();
    ...
    create_style_elements: function(styles_list){
        ...
        $(styles_list).each(function(k, v){
            if(v && v.trim() != ''){
                me.style_template.appendChild(document.createTextNode('#{{section}}' + v));
            }            
        });
    },
    copy_element: function(wrapper, type, selector, row_count, column_count, component_class){
        let me = this;
        let cls = selector.split('#{{section}} ')[1];
        if(type == 'Component'){
            let component_name = this.section_template.find(cls).find('.' + component_class).attr('data-component');
            ...
            let new_row = $(`<div class="row section-row sec-row${row_length}"></div>`);
            ...
            if($(node)[0].children.length > 0){
                for(var j=0; j< $(node)[0].children.length; j++){
                    ...
                    if($(col_node)[0].children.length > 0){
                        for(var k = 0; k < $(col_node)[0].children.length; k++){
                            let component_name = $($(col_node)[0].children[k]).attr('data-component');
                            let component = me.components.find(obj=>obj.name == component_name);
                            let html = me.update_component(component, (row_length), (j +1), `.sec-row${row_length}-col${j + 1}`, true);
    ...
    delete_element: function(wrapper, type, selector, row_count, column_count, component_class){
        let me = this;
        let cls = selector.split('#{{section}} ')[1];
        if(type == 'Component'){
            ...
            $(row_html).find('.section-comp').each(function(){
    ...
    update_component: function(component, row_count, column_count, cls, return_html=false){
        let me = this;
        let html = component.html;
        let component_length = $(me.section_template).find(cls).find('.section-comp').length + 1;
        if(component.content.length > 0){
            if(this.from_form){
                $(component.content).each(function(k, v){
                    let child_row = frappe.model.add_child(me.frm.doc, 'Section Content', 'content', me.frm.doc.content.length + 1);
                    ...
                    html = html.split(`{{${v.field_key}}}`)[0] + `{{${child_row.field_key}}}` + html.split(`{{${v.field_key}}}`)[1];
                });
                me.frm.refresh_field('content');
            }
        }
        let comp_cls = `sr${row_count}c${column_count}-com${component_length}`;
        let component_html = $(`<div class="section-comp ${comp_cls}" data-component="${component.name}">${html}</div>`);
        if(return_html == true)
    ...
    delete_component: function(cls, component_class){
        ...
        if(!cls.startsWith('.'))
            cls = '.' + cls;
        let html = this.section_template.find(cls).find('.' + component_class).html();
        let lsplit = html.split('{{');
        let names = [];
        $(lsplit).each(function(k, v){
            if(v.indexOf('}}') != -1){
                let key = v.split('}}')[0];
                let check_content = me.frm.doc.content.find(obj=>obj.field_key==key);
                if(check_content){
        ...
        this.section_template.find(cls).find('.' + component_class).remove();
    }
});