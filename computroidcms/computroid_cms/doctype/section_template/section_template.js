...
frappe.ui.form.on('Section Template', {
	...
	get_linked_documents: function (frm) {
		frappe.call({
			method: 'computroidcms.computroidcms.doctype.section_template.section_template.get_linked_documents',
			args: {
...
{% include 'computroidcms/computroidcms/doctype/section_template/section_content.js' %}
...
var print_builder = Class.extend({
    ...
    async get_sections() {
		...
		await frappe.call({
			method: '',
			type: "post",
			url: "/api/method/computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_section_templates",
			args: {
				device_type: "Web",
			},
			freeze: true,
