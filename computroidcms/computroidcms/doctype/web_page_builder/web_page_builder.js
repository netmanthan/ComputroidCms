...
frappe.require("assets/computroidcms/js/jquery.ui.min.js");
frappe.require("assets/computroidcms/js/uppy.min.css");
frappe.require("assets/computroidcms/js/uppy.min.js");
frappe.require("assets/computroidcms/css/jodit.min.css");
frappe.require("assets/computroidcms/js/jodit.min.js");
frappe.require("assets/computroidcms/js/jquery.slimscroll.min.js");
var is_row_updated = 0;
frappe.ui.form.on('Web Page Builder', {

    refresh: function(frm) {
        frm.r___id = Math.floor(Math.random() * 1000)
        // frm.trigger("store_global_font_and_weight_values")
        if(frm.doc.document && frm.doc.w_page_type=="List"){
           frm.set_query("data_fetch_from", function() {
            return {
                "query":"computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_linked_docs",
                "filters": {
           ...
           if(frm.doc.w_page_type=="List" && frm.doc.enable_side_menu==1){
               frappe.call({
                    method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_side_menu_fields',
                    args: {
        ...
        if(frm.doc.document && frm.doc.w_page_type=="Detail"){
              frappe.call({
                        method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_linked_fields',
                        args: {
                             "document": frm.doc.document
                        },
                        freeze: true,
                        callback: function(r) {
                            var options = [];
                            for (var i = 0; i < r.message.length; i++) {
                               options.push({"value": r.message[i].fieldname,"label":r.message[i].options})
                            }
                            frm.set_df_property("related_item_mapping_column", "options", options);
        ...
        if (!frm.__islocal) {
            ...
            if(frm.doc.w_page_type == "List" && frm.doc.list_style_image!=undefined){
                 $("#list_style_image").remove();
                 var img_html = '<img src="'+frm.doc.list_style_image+'" style="margin-top: 10px;" id="list_style_image"/>';
                 $('button[data-fieldname="choose_list_style"]').parent().append(img_html);
            }
            if(frm.doc.w_page_type == "Detail" && frm.doc.detail_layout_image!=undefined){
                 $("#detail_style_image").remove();
                 var img_html = '<img src="'+frm.doc.detail_layout_image+'" style="margin-top: 10px;" id="detail_style_image"/>';
                 $('button[data-fieldname="choose_detail_style"]').parent().append(img_html);
            }
        }
               ...
               var title_dialog =   new frappe.ui.Dialog({
                     ...
                     title_dialog.set_primary_action(__('Save As Template'), function() {
                         let val = title_dialog.get_values();
                          frappe.call({
                                method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.save_as_template',
                                args: {
    ...
    store_global_font_and_weight_values(frm){
        console.log(">>triger<<")
        frappe.db.get_value('Web Theme', {is_active: 1}, 'name')
        .then(r => {
            if(r && r.message && r.message.name){
                frappe.call({
                    method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_global_fonts',
                    args: {
                        parent:r.message.name
                    },
                    freeze: true,
                    callback: function(r) {
                       console.log(">> Global fonts<<",r)
                       if(r && r.message && r.message.length > 0){
                        r.message.map(res =>{
                            if(res.title == "Primary"){
                                // frm.primary_font_family = 
                                // frm.primary_font_weight = 
                            }
                            else if(res.title == "Secondary"){
                                // frm.secondary_font_family = 
					            // frm.secondary_font_weight = 
                            }
                            else if(res.title == "Text"){
                                // frm.text_font_family = 
                                // frm.text_font_weight = 
                            }
                            else if(res.title == "Accent"){
                                // frm.accent_font_family = 
					            // frm.accent_font_weight = 
                            }
                        })
                       }
                    }
    ...
    choose_list_style:function(frm){
     ...
     choose_detail_style:function(frm){
        new add_detail_template({
            frm: frm,
        });
    },
    ...
    edit_with_go1_builder:function(frm){
        window.location.href="/app/computroidcms-builder?route="+frm.doc.route;
    },
        ...
        if (frm.doc[field] && frm.doc[field].length > 0) {
            frm.doc[field].map(f => {
                ...
                let row = $(`<tr data-id="${f.name}">
                    <td style="width: 50%;"><img src="/assets/computroidcms/images/section-icon.svg" style="height:30px;cursor: all-scroll;position: relative;top: 8px;margin-right: 10px;"> ${section_data}</td>
    ...
    delete_sections: function(frm, name, parentfield) {
        frappe.confirm(
            ...
            function(){
              frappe.call({
                    method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.delete_section',
                    args: {
                        name: name,
    ...
    edit_styles_sections: function(frm, name, parentfield) {
        var id=name;
        frappe.call({
                method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_section_properties',
                ...
                callback: function(r) {
                    if(r.message.styles){
                       ...
                       styledialog.show();
                        ...
                        $(gradient_html).insertBefore(styledialog.$wrapper.find('[data-fieldname="bg-gradient-color"] button'))
                        ...
                        $(mask_html).insertBefore(styledialog.$wrapper.find('[data-fieldname="bg-mask-color"] button'))
                        ...
                        $(pattern_html).insertBefore(styledialog.$wrapper.find('[data-fieldname="bg-pattern-color"] button'))
                        ...
                        $(divider_html).insertBefore(styledialog.$wrapper.find('[data-fieldname="bg-divider-img"] button'))

                        styledialog.$wrapper.find('[data-fieldname="bg-gradient-color"] button').on("click",(e) =>{
                            // console.log(">> gradinat color working<<")
                            poup_bg_dialog('Gradient')
                        })
                        styledialog.$wrapper.find('[data-fieldname="bg-pattern-color"] button').on("click",(e) =>{
                            // console.log(">> pattern color working <<")
                            poup_bg_dialog('Pattern')
                        })
                        styledialog.$wrapper.find('[data-fieldname="bg-mask-color"] button').on("click",(e) =>{
                            // console.log(">> mask color working <<")
                            poup_bg_dialog('Mask')
                        })
                        
                        styledialog.$wrapper.find('[data-fieldname="bg-divider-img"] button').on("click",(e) =>{
                            // console.log(">> mask color working <<")
                       ...
                       styledialog.set_primary_action(__('Save'), function() {
                        ...
                        //    console.log(">> Submitted value <<",values)
                           ...
                           for (let k in values) {
                                ...
                                if(values[k]!="" && values[k]!="0px" && values[k]!="0" && k!="background_options" && k!="bg-divider-img"){
                                ...
                                if (k == "font-size" && enable_font_media_query) {
                                        Object.keys(fonts_media_query_data).map(e_res => {
                                            if (e_res == "pc-font-size") {
                                                font_size_css_design += `@media (min-width: 992px){.${r.message.class_name}{font-size:${fonts_media_query_data[e_res].split('-').join('')+" !important;"}}}`
                                            }
                                            else if (e_res == "mbl-font-size") {
                                                font_size_css_design += `@media (max-width: 767px){.${r.message.class_name}{font-size:${fonts_media_query_data[e_res].split('-').join('')+" !important;"}}}`
                                            }
                                            else if (e_res == "tab-font-size") {
                                                font_size_css_design += `@media (max-width: 992px){.${r.message.class_name}{font-size:${fonts_media_query_data[e_res].split('-').join('')+" !important;"}}}`
                                            }
                                        })
                                    }
                            }
                            ...
                            if(popup_variable_color_codes.hasOwnProperty('bg-divider-img')){
                             ...
                             css_design +="}";
                             if(divider_css_design){
                                css_design += "."+r.message.class_name+` .divider{ ${divider_css_design}`+'}'
                             }
                             if(font_size_css_design){
                                css_design += font_size_css_design
                             }
                            //  console.log(">> final submitted css <<",css_design)
                            $('[data-fieldname="'+id+'"]').attr("css_design",css_design)
                            $('[data-fieldname="'+id+'"]').attr("style_json",JSON.stringify(style_json))
                             frappe.call({
                                method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.update_section_properties',
                       ...
                       for (var i = 0; i < units_sections.length; i++) {
                            ...
                            styledialog.$wrapper.find(".form-control[data-fieldname='" + units_sections[i] + "']").parent().find("select").val((units_sections_field_with_units[i] && units_sections_field_with_units[i].unit)? units_sections_field_with_units[i].unit:"px")
                       }

                       if(enable_font_media_query){
                           styledialog.$wrapper.find('[data-fieldname="font-size"] label').parent().css({ "display": "flex", "gap": "10px", "align-items": "center" })
                           styledialog.$wrapper.find('[data-fieldname="font-size"] label').parent().css({ "display": "flex", "gap": "10px", "align-items": "center" })
                           styledialog.$wrapper.find('[data-fieldname="font-size"] label').parent().append(`<i id="mbl-font-icon" style="font-size:20px;cursor:pointer;padding-bottom:3px;" class="fa fa-mobile" aria-hidden="true"></i>
                        ...
                        <i id="pc-font-icon" style="font-size:15px;cursor:pointer;font-weight: 700;color: #0101ff;" class="fa fa-desktop" aria-hidden="true"></i>`)
                           styledialog.$wrapper.find('[data-fieldname="font-size"] input').attr("input_value", "Web")
                           styledialog.$wrapper.find('[data-fieldname="font-size"] #mbl-font-icon').on("click", () => {
                               // console.log(">>mbl working<<")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] input').attr("input_value", "Mobile")
                               if (fonts_media_query_data['mbl-font-size']) {
                                   styledialog.$wrapper.find('[data-fieldname="font-size"] input').val(fonts_media_query_data['mbl-font-size'].split('-')[0])
                                   styledialog.$wrapper.find(".form-control[data-fieldname='font-size']").parent().find("select").val(fonts_media_query_data['mbl-font-size'].split('-')[1] ? fonts_media_query_data['mbl-font-size'].split('-')[1] : "px")
                               }
                               else {
                                   styledialog.$wrapper.find('[data-fieldname="font-size"] input').val('')
                               }
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #mbl-font-icon').attr("style", "font-size:22px;cursor:pointer;font-weight: 700;color: #0101ff;")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #pc-font-icon').attr("style", "font-size:15px;cursor:pointer;")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #tab-font-icon').attr("style", "font-size:15px;cursor:pointer;")
                           })
                           styledialog.$wrapper.find('[data-fieldname="font-size"] #tab-font-icon').on("click", () => {
                               // console.log(">>tab working<<")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] input').attr("input_value", "Tab")
                               if (fonts_media_query_data['tab-font-size']) {
                                   styledialog.$wrapper.find('[data-fieldname="font-size"] input').val(fonts_media_query_data['tab-font-size'].split('-')[0])
                                   styledialog.$wrapper.find(".form-control[data-fieldname='font-size']").parent().find("select").val(fonts_media_query_data['tab-font-size'].split('-')[1] ? fonts_media_query_data['tab-font-size'].split('-')[1] : "px")
                               }
                               else {
                                   styledialog.$wrapper.find('[data-fieldname="font-size"] input').val('')
                               }
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #tab-font-icon').attr("style", "font-size:17px;cursor:pointer;font-weight: 700;color: #0101ff;")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #pc-font-icon').attr("style", "font-size:15px;cursor:pointer;")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #mbl-font-icon').attr("style", "font-size:20px;cursor:pointer;")
                           })
                           styledialog.$wrapper.find('[data-fieldname="font-size"] #pc-font-icon').on("click", () => {
                               // console.log(">>pc working<<")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] input').attr("input_value", "Web")
                               if (fonts_media_query_data['pc-font-size']) {
                                   styledialog.$wrapper.find('[data-fieldname="font-size"] input').val(fonts_media_query_data['pc-font-size'].split('-')[0])
                                   styledialog.$wrapper.find(".form-control[data-fieldname='font-size']").parent().find("select").val(fonts_media_query_data['pc-font-size'].split('-')[1] ? fonts_media_query_data['pc-font-size'].split('-')[1] : "px")
                               }
                               else {
                                   styledialog.$wrapper.find('[data-fieldname="font-size"] input').val('')
                               }
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #pc-font-icon').attr("style", "font-size:17px;cursor:pointer;font-weight: 700;color: #0101ff;")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #mbl-font-icon').attr("style", "font-size:20px;cursor:pointer;")
                               styledialog.$wrapper.find('[data-fieldname="font-size"] #tab-font-icon').attr("style", "font-size:15px;cursor:pointer;")
                           })
                           $(styledialog.$wrapper.find('[data-fieldname="font-size"] input')).on("keyup", (e) => {
                               let entered_value = $(e.target).val()
                               let unit = $(styledialog.$wrapper.find('[data-fieldname="font-size"] select')).val()
                               let input_value = $(styledialog.$wrapper.find('[data-fieldname="font-size"] input')).attr("input_value")
                           ...
                           $(styledialog.$wrapper.find('[data-fieldname="font-size"] select')).on("change", (e) => {
                        ...
                        function poup_bg_dialog(bg_type){
                            frappe.call({
                                method:"computroidcms.computroidcms.doctype.web_theme.web_theme.get_color_code_data",
                                ...
                                if(temp_html){
                                    temp_html.click((e) =>{
                                        ...
                                        if($(e.target).attr("bg_type") == 'Gradient'){
                                            ...
                                            styledialog.$wrapper.find('[data-fieldname="bg-gradient-color"] .gradient_modify_sec .btn-previw-section .gradient-color').css({"background":`${$(e.target).attr("value").split(";")[0]}`})
                                            bg_dialog.hide();
                                        }
                                        else if($(e.target).attr("bg_type") == 'Pattern'){
                                            popup_variable_color_codes["bg-pattern-color"] = $(e.target).attr("value")
                                            styledialog.$wrapper.find('[data-fieldname="bg-pattern-color"] .pattern_modify_sec .btn-previw-section .pattern-color').css({"background":`${$(e.target).attr("value")}`})
                                            bg_dialog.hide();
                                        }
                                        else if($(e.target).attr("bg_type") == 'Mask'){
                                            popup_variable_color_codes["bg-mask-color"] = $(e.target).attr("value")
                                            styledialog.$wrapper.find('[data-fieldname="bg-mask-color"] .mask_modify_sec .btn-previw-section .mask-color').css({"background":`${$(e.target).attr("value")}`})
                                            bg_dialog.hide();
                                        }
                                            ...
                                            styledialog.$wrapper.find('[data-fieldname="bg-divider-img"] .divider_modify_sec .btn-previw-section .divider-color').css({"-webkit-mask":`url('${$(e.target).attr("value")}')no-repeat center / contain`,"background":`${divider_background}`})
            ...
            // function svg_to_png_coverter(svgText, margin,fill){
...
var add_new_section = Class.extend({
    ...
    get_all_sections: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_sections',
            args: {
    ...
    get_section_templates: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_section_templates',
            args: {
                device_type: me.device_type
            },
            async: false,
    ...
    make_dialog: function() {
        ...
        //             }
        //             frappe.call({
        //                 method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.convert_template_to_section',
        //                 args: {
...
function bind_layout_data(wrapper){
    frappe.call({
        method: "computroidcms.computroidcms.doctype.section_template_layout.section_template_layout.get_layout_data",
        args: {},
        freeze: true,
        callback: function (r) {
...
var modify_section_data = Class.extend({
    ...
    get_data_fields: function() {
        let me = this;
         // console.log(me,"me----")
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_section_content',
            args: {
                section: me.section,
    ...
    get_patterns: function() {
        let patterns = []
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_patterns_list',
            args: {},
            async: false,
            callback: function(r) {
                if (r.message) {
    ...
    make_dialog: function() {
        ...
        // }
        // console.log(this.fields.content,"this.fields.content")
       // if(this.child_sections){
       //      for (var i = 0; i < this.child_sections.length; i++) {
       //           var child_section_html = '<label class="control-label" style="padding-right: 0px;float:left;width:100%">'+this.child_sections[i].label+'</label><div style="float:left;width:100%;margin-bottom:15px">';
       //           for (var j = 0; j < this.child_sections[i].child_properties.length;j++) {
       //               var default_value =this.child_sections[i].child_properties[j].default;
       //               if(this.fields.css_json){
       //                for (const [key, value] of Object.entries(this.fields.css_json)) {
       //                  if (key==this.child_sections[i].child_properties[j].fieldname){
       //                      default_value=value;
       //                  }
       //              }
       //          }
       //               child_section_html+='<div style=" width: calc('+(100/this.child_sections[i].child_properties.length)+'% - 10px);float: left;margin-right: 5px;margin-left: 5px;">';
       //               child_section_html+='<label class="control-label">'+this.child_sections[i].child_properties[j].label+'</label>';
       //               if(this.child_sections[i].child_properties[j].fieldtype=="Data"){
       //                   child_section_html+='<input type="text" autocomplete="off" class="input-with-feedback form-control" maxlength="140" data-fieldtype="Data" data-fieldname="'+this.child_sections[i].child_properties[j].fieldname+'" value="'+default_value+'" placeholder="" >';
       //               }
       //               if(this.child_sections[i].child_properties[j].fieldtype=="Color"){
       //                   child_section_html+='<input type="text" autocomplete="off" class="input-with-feedback form-control" data-fieldtype="Color"  data-fieldname="'+this.child_sections[i].child_properties[j].fieldname+'" value="'+default_value+'" placeholder="Choose a color" >';
       //               }
       //               if(this.child_sections[i].child_properties[j].fieldtype=="Select"){
       //                   child_section_html+='<select type="text" autocomplete="off" class="input-with-feedback form-control" maxlength="140" data-fieldtype="Select" data-fieldname="'+this.child_sections[i].child_properties[j].fieldname+'" placeholder="">';
       //                   for (var o = 0; o < this.child_sections[i].child_properties[j].options.length; o++) {
       //                       var selected = '';
       //                       if(this.fields.css_json){
       //                        for (const [key, value] of Object.entries(this.fields.css_json)) {

       //                          if (value==this.child_sections[i].child_properties[j].options[o]){
       //                              selected='selected';
       //                          }
       //                      }}
       //                       child_section_html+='<option value="'+this.child_sections[i].child_properties[j].options[o]+'" '+selected+'>'+this.child_sections[i].child_properties[j].options[o]+'</option>'
       //                   }
       //                   child_section_html+='</select>';
       //               }
       //               child_section_html+='</div>';
       //           }
       //           child_section_html+='</div>';
       //           this.dialog.$wrapper.find('div[data-fieldname="'+this.child_sections[i].fieldname+'"]').append(child_section_html)
       //      }
       //  }
       //  if(this.child_sections){
       //       this.dialog.$wrapper.find('div[data-fieldname="is_full_width"]').attr("style","float:left;width:100%;")
       ...
       //          this.dialog.$wrapper.find('.form-layout').css("background", "#fff");
       //          this.dialog.$wrapper.find('.form-layout').css("width", "calc(100% + 30px)");
       //          this.dialog.$wrapper.find('.form-page-section .row.form-section.visible-section').css("width", "70%");
       //          this.dialog.$wrapper.find('.form-page-section .row.form-section.visible-section').css("float", "left");
       //          this.dialog.$wrapper.find('.form-page-section .row.form-section.visible-section:first-child').css("width", "30%");
       //          this.dialog.$wrapper.find('.form-page-section .row.form-section.visible-section:first-child').css("border-right", "1px solid #ddd");
       //          this.dialog.$wrapper.find('.form-page-section .row.form-section.visible-section:first-child').css("min-height", "720px");
       //          this.dialog.$wrapper.find('.form-page-section .row.form-section.visible-section:first-child').find("h6").attr("style","margin-left: -20px;margin-top: -15px;background-color: #f5f5f5;width: calc(100% + 42px);padding: 10px 15px;")
       //          this.dialog.$wrapper.find('.row.form-section.tabbreak-head.tab-head.visible-section').css("padding","0")
       ...
       //  }
        ...
        for (var value of this.fields.content) {
            if(value['allow_update_to_style']==1){
                ...
                // if(value['css_properties_list'].length>0){
                  ...
                  // }
                // columnbreak_design
                // this.dialog.fields_dict.columnbreak_design.wrapper.show();
                // let elements_html = this.dialog.fields_dict.elements_style.$wrapper.empty();
                // $(`<p>hi</p>`).append(elements_html)
                //added by boopathy



                //end
        //         let header_value = value['field_label']
        //         header_value = header_value.toUpperCase();
        //         let css_value =this.dialog.$wrapper.find('.row.form-section.visible-section').find(".section-head").find(".h6").text()
        //         $(this.dialog.$wrapper.find('.row.form-section.visible-section').find(".section-head").find(".h6")).each((index, element) => {
        //            if($(element).text()==value['field_label']){
        //                 $(element).append(` <span id="css_click" value=${value['css_properties_list']}><i class="fa fa-cogs"></i></span>`)
        //                 // $(element).css("background-color","yellow")
        //                 $(element).find('#css_click').on( "click", function(event) {
        //                     console.log(value,"Success");

        //                     let styles = value['css_properties_list']
        //                     console.log(styles,"styles")
        //                     let field_list =[]
        //                     field_list.push({ 'fieldtype': 'Section Break', 'fieldname': 'sb_design2', 'label': 'Design', 'collapsible': 0 })
        ...
        //                     }
        //                     let field_dialog = new frappe.ui.Dialog({
        //                         title: __(title),
        //                         fields: field_list
        //                     });
        //                     field_dialog.show()
    ...
    get_blog_category:function(type){
          ...
          if(type=='shuffle'){
              if(is_dynamic_data_val==0 && reference_name_val!=""){
                frappe.call({
                    method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_shuffled_blog_category',
                    args: {
                    ...
                    callback: function(r) {
                        ...
                        if(r.message){
                               ...
                               for (var i = 0; i < r.message.length; i++) {
                                   ...
                                   row ="<tr><td><img src='/assets/computroidcms/images/section-icon.svg' style='height:30px;cursor: all-scroll;position: relative;top: 0;margin-right: 10px;text-align: center;left: 10px;'></td><td style='width:200px'>"+r.message[i].name+"</td><td>"+image_html+r.message[i].title+"</td></tr>";
                                   t_html.append(row)
                               }
           ...
           for (var i = 0; i < json_data.length; i++) {
               ...
               if(json_data[i].thumbnail_image!=null && json_data[i].thumbnail_image!=undefined){
                ...
                row ="<tr><td><img src='/assets/computroidcms/images/section-icon.svg' style='height:30px;cursor: all-scroll;position: relative;top: 0;margin-right: 10px;text-align: center;left: 10px;'></td><td style='width:200px'>"+json_data[i].name+"</td><td>"+image_html+json_data[i].title+"</td></tr>";
               t_html.append(row)
           }
           t_html.appendTo(p_html);
           me.dialog.$wrapper.find('[data-fieldname="blog_category_html"]').parent().parent().parent().parent().show();
    ...
    get_category_products:function(type){
          ...
          if(type=='shuffle'){
              if(is_dynamic_data_val==0 && reference_name_val!=""){
                frappe.call({
                    method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_shuffled_category_products',
                    args: {
                        category: reference_name_val,
                        no_of_records:no_of_records_val
                    },
                    ...
                    callback: function(r) {
                        ...
                        if(r.message){
                               ...
                               for (var i = 0; i < r.message.length; i++) {
                                   ...
                                   row ="<tr><td><img src='/assets/computroidcms/images/section-icon.svg' style='height:30px;cursor: all-scroll;position: relative;top: 0;margin-right: 10px;text-align: center;left: 10px;'></td><td style='width:200px'>"+r.message[i].name+"</td><td>"+image_html+r.message[i].item+"</td></tr>";
                                   t_html.append(row)
                               }
                               t_html.appendTo(p_html);
           ...
           for (var i = 0; i < json_data.length; i++) {
               ...
               if(json_data[i].product_image!=null && json_data[i].product_image!=undefined){
                ...
                row ="<tr><td><img src='/assets/computroidcms/images/section-icon.svg' style='height:30px;cursor: all-scroll;position: relative;top: 0;margin-right: 10px;text-align: center;left: 10px;'></td><td style='width:200px'>"+json_data[i].name+"</td><td>"+image_html+json_data[i].item+"</td></tr>";
               t_html.append(row)
           }
           t_html.appendTo(p_html);
           me.dialog.$wrapper.find('[data-fieldname="category_products_html"]').parent().parent().parent().parent().show();
    ...
    get_fields_list: function() {
        ...
        if (this.fields.section_type == 'Predefined Section' && !this.fields.dynamic_data && !this.fields.is_login_required) {
            ...
            // frappe.call({
            //     method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_featured_products',
            //     args: {},
            //     async: false,
            //     callback: function(r) {
            //         data = r.message
    ...
    save_data: function() {
        ...
        //     css_design +="}"
            ...
            //     frappe.call({
            //         method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.generate_css_file',
            //         args: {
        ...
        if (results.length > 0 || me.list_section_data.length > 0) {
            console.log(results)
            frappe.call({
                method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.update_section_content',
                args: {
                    docs: results,
    ...
    update_design: function() {
        ...
        if (this.selected_pattern) {
            ...
            frappe.call({
                method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.update_patterns',
        ...
        if (!section_data || section_data == '')
            ...
            table.find('tbody').append(row);
                    ...
                    row.find('.btn-danger').click(function() {
                        ...
                        frappe.call({
                            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.update_featured_item',
                            args: {
                                doctype: me.fields.reference_document,
                                name: val.preitem,
                                checked: 1,
    ...
    get_random_records: function(dt, records) {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_random_records',
            args: {
                dt: dt,
                records: records,
                business: cur_frm.doc.business
    ...
    get_predefined_records: function(name, dt, no_of_records, business) {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_predefined_records',
            args: {
                dt: dt,
    ...
    get_collections_records: function(collections) {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_collection_records',
            args: {
    ...
    get_image_album: function(dt, dn) {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_image_album',
            args: {
                dt: dt,
                dn: dn,
                business: cur_frm.doc.business
    ...
    upload_component: function(target, progress) {
        ...
        var uppy = Uppy.Core({
            ...
            .use(Uppy.XHRUpload, {
                endpoint: window.location.origin + '/api/method/computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.upload_img',
                method: 'post',
                formData: true,
                fieldname: 'file',
                headers: {
...
var add_page_template = Class.extend({
    ...
    get_page_templates: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_page_templates',
            args: {
            ...
            item.click(function() {
                if(cur_frm.doc.page_title!="" && cur_frm.doc.page_title!=undefined){
                    ...
                    frappe.confirm('Are you sure you want to proceed?',
                        () => {
                             frappe.call({
                                    method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.import_sections_from_template',
                                    args: {
...
var add_list_template = Class.extend({
    ...
    get_page_templates: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_list_page_templates',
            args: {
            },
            async: false,
            callback: function(r) {
...
var columns_mapping = Class.extend({
    ...
    get_section_columns: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_section_columns',
            args: {
...
var add_detail_template = Class.extend({
    init: function(opts) {
        this.frm = opts.frm;
        this.make();
    },
    ...
    get_page_templates: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_detail_page_templates',
            args: {
            },
            async: false,
            callback: function(r) {
    ...
    section_html: function() {
        let me = this;
        let wrapper = this.dialog.fields_dict.section_html.$wrapper.empty();
        let html = $(`<div class="row" id="DetailPageList"></div>
            <style>
                div[data-fieldname="section_html"] #DetailPageList .section-title{
                    padding: 30% 2%; text-align: center; height: 205px;
                }
                div[data-fieldname="section_html"] #DetailPageList .section-item.active{
                    border: 1px solid #0c9e0c; background: #efefef1f; color: #0c9e0c;
                }
                div[data-fieldname="section_html"] #DetailPageList .section-img{
                    position: relative; height: 203px;
                }
                div[data-fieldname="section_html"] #DetailPageList .section-img img{
                    position: absolute; top: 50%; left: 50%; max-height: 190px;
                    transform: translate(-50%, -50%); vertical-align: middle;
                }
                div[data-fieldname="section_html"] #DetailPageList .section-item{
                    margin-bottom: 10px; border: 1px solid #ddd; background: #f3f3f3; cursor:pointer;
                }
                div[data-fieldname="section_html"] #DetailPageList .section-item p{
                    text-align: center;
                }
                li.active{background-color:#f3f3f3;}
        ...
        let data = me.sections ? me.sections : me.templates;
        wrapper.find('#DetailPageList').append("<div class='col-md-12' id='sec-right-content' style='padding-top: 15px;height: 510px;overflow-y: auto;'></div>");
        data.map(f => {
            let template = `<div class="section-title">${f.title}</div>`;
            if (f.image) {
                template = `<div class="section-img" ><img src="${f.image}" /></div><p>${f.title}</p>`
            }
            let item = $(`<div class="col-md-4 col-sm-6 col-xs-6" style="float:left" >
                    <div class="section-item">${template}</div>
                </div>`);
            wrapper.find('#DetailPageList').find("#sec-right-content").append(item)
            item.click(function() {
                // if(cur_frm.doc.page_title!="" && cur_frm.doc.page_title!=undefined){
                    me.selected_section = { 'template': f.name }
                    $("#detail_style_image").remove();
                    me.frm.doc.detail_layout = f.name;
                     me.frm.doc.detail_layout_json = f.layout_json;
                     me.frm.doc.detail_layout_image = f.image;
                     var img_html = '<img id="detail_style_image" src="'+f.image+'" style="margin-top: 10px;"/>';
                      $('button[data-fieldname="choose_detail_style"]').parent().append(img_html);
                    me.dialog.hide();
