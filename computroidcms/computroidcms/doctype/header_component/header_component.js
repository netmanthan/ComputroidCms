...
frappe.ui.form.on('Header Component', {
    ...
    delete_sections: function(frm, name, parentfield) {
	 ...
	 pick_layout(frm){
		frappe.call({
			method:"computroidcms.computroidcms.doctype.header_component.header_component.get_header_layouts",
			args:{},
			freeze:true,
			callback:(res) =>{
	...
	open_footer_popoup(frm,layouts){
		...
		<label style="cursor:pointer;width:100%;" value="${each_p.name}">
			...
			<input type="radio" value="${each_p.name}" name="theme_radio" onclick="select_unselect_layout($(this))" style="position:absolute;top:6px;right:30px;z-index:99;">
		</label></div>`
...
var add_new_section = Class.extend({
    ...
    get_section_templates: function() {
        let me = this;
        frappe.call({
            method: 'computroidcms.computroidcms.doctype.web_page_builder.web_page_builder.get_header_section_templates',
            args: {
                device_type: me.device_type
            },
            async: false,
