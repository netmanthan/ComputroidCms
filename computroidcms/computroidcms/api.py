...
def get_template_folder(url=None, business=None, temp=0):
	try:
		if not url:
			path = frappe.get_module_path("computroidcms")
...
def check_domain(domain_name):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'computroidcms.computroidcms.api.check_domain')
...
def get_page_content(route=None, user=None, customer=None, domain=None,business=None, application_type="mobile"):
	...
	if page_type == "List":
		...
		if check_builder:
			...
			columns = columns[:-1]
			list_content = frappe.db.sql(""" SELECT {columns} FROM `tab{document}` doc {condition} {order_by}""".format(condition=condition,order_by=order_by,columns=columns,document=check_builder[0].document),as_dict=1)
...
def get_detail_page_content(route):
 	check_dt = frappe.db.sql(""" select D.name from `tabDocType` D inner join  where has_web_view=1 """,as_dict=1)
 	for x in check_dt:
 		pass
...
def update_website_context(context):
	try:
		...
		frappe.log_error(frappe.get_traceback(),"computroidcms.computroidcms.api.update_website_context")
...
def get_device_type(context):
	try:
		...
		context.device_type="Desktop"
		frappe.log_error(frappe.get_traceback(), "computroidcms.computroidcms.api.get_device_type") 	
